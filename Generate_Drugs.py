...
GEN_MODEL_REVISION = "f42a5a10e24c0350aeadb50865bd90a714d0b2bf"
...
tokenizer = GPT2TokenizerFast.from_pretrained(GEN_MODEL_NAME, revision=GEN_MODEL_REVISION, max_len=256)
gen_model = GPT2LMHeadModel.from_pretrained(GEN_MODEL_NAME, revision=GEN_MODEL_REVISION).to(device).eval()
...
known_smiles = set()
for smi in pd.read_csv(PROCESSED_DIR / "Response_processed.csv")["smiles"].astype(str).unique():
    mol = Chem.MolFromSmiles(smi)
    if mol is not None:
        known_smiles.add(Chem.MolToSmiles(mol))
...
validity = valid_count / max(len(raw_smiles), 1)
print(f"Sampled {len(raw_smiles)} raw sequences -> {valid_count} valid ({validity:.1%}) "
      f"-> {len(valid_smiles)} novel, unique SMILES")
