...
# ------------------------
# Load the TransCDR paper data set
# ------------------------
...
gdsc = pd.concat([gdsc, transcdr], ignore_index=True)
gdsc = gdsc.dropna(subset=['COSMIC_ID', 'cell_type', 'drug_name', 'lnIC50'])
gdsc = (gdsc.groupby(['COSMIC_ID', 'cell_type', 'drug_name', 'assay_name', 'GDSC_tissue'], as_index=False, dropna=False).agg(lnIC50=('lnIC50', 'median')))
...
smiles["smiles"] = smiles["smiles"].map(lambda value: Chem.MolToSmiles(mol) if (mol := Chem.MolFromSmiles(str(value))) is not None else np.nan)
...
# Exclude ambiguous normalized names instead of silently taking the first model.
ambiguous_names = model_lookup.groupby('norm_name')['depmap_id'].nunique()
ambiguous_names = ambiguous_names[ambiguous_names > 1].index
if len(ambiguous_names):
    print(f"Excluded {len(ambiguous_names)} ambiguous normalized cell-line names")
model_lookup = model_lookup[~model_lookup['norm_name'].isin(ambiguous_names)].drop_duplicates(subset=['norm_name'])
